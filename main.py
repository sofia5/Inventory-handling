...
def main():
    ...
    while True:
        print("\n" + tabulate([['S[quantity] [product ID]', 'To sell products, enter S + number of items to sell + SPACE + product ID'],
                               ['I[quantity] [product ID]', 'To buy, enter I + number of items to buy + SPACE + product ID.'], [
            'L', 'Enter L to show the current product status.'], ['C', 'Enter C to show the product catalogue.'],
            ['T', 'Toggle automatic purchasing. Current status is ' + (
                'On' if autoPurchase else 'Off') + '. \nThis means that when X number of items are sold 2X are bought.'],
            ['D[every n:th free]',
             'Handle discounts given. Currently every n:th item (of the same product) is given for free. Does not include product packages.'],
            ['Q', 'Enter Q to quit.']], colalign=("center",), headers=['Operation', 'Description']))
        ...
        if len(choice) == 0:
            ...
            # Quit program
            if operation == "Q" and len(choice) == 1:
                print("Closing application")
                exit(0)

            # Show inventory
            elif operation == "L" and len(choice) == 1:
            ...
            elif operation == "C" and len(choice) == 1:
                res = productHandler.getProducts()
                ids = []
                names = []
                fruits = []
            ...
            elif operation == "T" and len(choice) == 1:
                autoPurchase = productHandler.toggleAutoPurchase(autoPurchase)
                print("Changing automatic purchasing to " +
                      ('On' if autoPurchase else 'Off'))

            elif operation == "D" and isinstance(int(choice[1:]), int):
                discount = int(choice[1:])
                print("Changing discount to " + str(discount) + ". Buy " +
                      str(discount - 1) + ", get the next for free.")
            ...
            # Check productID and number of items as both are required to sell and buy
                ...
                if productID in productHandler.getProducts():
                    ...
                    print(
                        "Sorry, the operator or product ID provided is incorrect. Check the product catalogue (C) to get some help.")
